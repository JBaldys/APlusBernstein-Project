...
def remove_missing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    names_to_remove = df_missing[df_missing["missing_pct"] > 0.9]["factor"]
    return df[[col for col in df.columns if col not in names_to_remove]]


if __name__ == "__main__":
    df = read_sheet(1).compute()
    df = remove_missing(df)
    df.to_csv(processed_data_dir / "df_removed_missing.csv", index=False)
